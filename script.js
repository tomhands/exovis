//(C) Tom Hands, University of Leicester, tom.hands@leicester.ac.uk, 2013
/* This file is provided with no warranty of any sort. In general much of the code in here is hacked together and not the best coding practice*/
...
function Planet(names, axis, temp, ecc, mass, year, radius, meth, desc, period, img, imgDesc)
{
	this.names = names;
	this.axis = axis;
    this.axisInferred = false;
    this.radInferred = false;
	this.temp = temp;
	this.size = size;
		...
		xml.children("star").each(function()
		{
			...
			$(this).children("planet").each(function()
			{
				...
				if($(this).children("semimajoraxis").length)
				    ...
				    {
					planAxis = parseFloat($(this).children("period").text())/(365 * 2 * 3.142);
					planAxis *= planAxis * G * starMass;
					planAxis = Math.pow(planAxis, 1.0/3.0);
					axisInferred = true;
				    }
				var planEcc  = 0.0;
				if($(this).children("eccentricity").length)
				    {
					if($(this).children("eccentricity").attr("upperlimit") !== undefined)
					    planEcc = $(this).children("eccentricity").attr("upperlimit");
					else if($(this).children("eccentricity").attr("lowerlimit") !== undefined)
					    planEcc = $(this).children("eccentricity").attr("lowerlimit");
					else
					    planEcc = $(this).children("eccentricity").text();
				    }
				var planSize = 0.0;
			    var radInferred = false;
				if($(this).children("radius").length)
					planSize = $(this).children("radius").text();
			    else if($(this).children("mass").length)
			    {
				planSize = Math.pow(parseFloat($(this).children("mass").text())/0.00314635457, 1.0/2.06)/11.209;
				radInferred = true;
			    }
				var period = 0.0;
			    ...
			    planets[planets.length - 1].axisInferred = axisInferred;
			    planets[planets.length - 1].radInferred = radInferred;
			});
...
Game.prototype.updatePlanetInfo = function(i)
{
	...
	if(system.stars[0].planets[i].axis && !system.stars[0].planets[i].axisInferred)	
		$("#axis").text(sanitize(system.stars[0].planets[i].axis));
	else
		$("#axis").text("Unknown");
		
	if(system.stars[0].planets[i].radius && !system.stars[0].planets[i].radInferred)	
	...
	{
		$("#desc").append("<br>");
		$("<img>", { src: "open_exoplanet_catalogue/images/" + system.stars[0].planets[i].img  + ".jpg", width:240, title: system.stars[0].planets[i].imgDesc}).appendTo("#desc");
	}
...
Game.prototype.updateInfo = function(){
	if(systemList.populated)
	{
		...
		if(system !== true)
		{
			...
			{
				var planetElement = $("<li></li>");
			    var planetLink = $("<a title=\"Click to view more information about this planet.\">" + system.stars[0].planets[i].names[0] +"</a>");
			    planetLink.appendTo(planetElement);
				planetLink.mouseenter(generateHighlighter(no, i));
				planetLink.mouseleave(function(){if(!highlight.dontcancel) highlight.sys = highlight.plan = false;});
				planetLink.click(generatePlanetInfo(no, i));
		...
		$("#tweetdiv").text("");
		$("#tweetdiv").append("<a href=\"https://twitter.com/share\" class=\"twitter-share-button\" data-url=\"http://tomhands.com/exovis?sysname="+ encodeURIComponent(systemList.friendlyNames[no])+"\" data-text=\"I'm looking at the planetary system '"+ systemList.friendlyNames[no]+"' on @exo_vis!\" data-size=\"large\">Tweet</a>");
		twttr.widgets.load();
	}
};
...
Game.prototype.drawSystem = function(system, color, viewPortHeight, checkPlanet)
{
	...
	for(var i = 0; i < system.stars[0].planets.length; i++)
	{
	    ...
	    var aoverb = planet.axis/planet.semiMinor;
	    var raoverb = radius * aoverb;
	    var oneminusecc = ( 1 - planet.eccentricity*planet.eccentricity );
		this.ctx.save(); //Save canvas state before we transform it
		this.ctx.scale(aoverb, 1); //Scale canvas so we get an ellipse with it's semi major axis alinged on the browsers x axis
		this.ctx.beginPath();
		...
		//Get current r
		var r1 = raoverb * oneminusecc/(1 + planet.eccentricity*Math.cos(planet.theta));
		var thetaDot = Math.PI * radius * (raoverb)/(r1*r1*framesPerOrbit)* Math.pow(viewPortHeight/planet.axis, 1.5);
	    //console.log("frames per orbit " +planet.names[0] + " " +framesPerOrbit*Math.pow(planet.axis/viewPortHeight, 1.5));
		planet.theta += thetaDot;
		//Calculate X and Y coodrinates from theta
		var cosTheta = Math.cos(planet.theta);
		planet.displayR = raoverb * oneminusecc/(1 + planet.eccentricity*cosTheta);
...
Game.prototype.render = function(){
	if(systemList.populated === true)
	{
		...
		if(transitioning)
		{
			...
			{
				highlight.sys = highlight.plan = highlight.dontcancel = false;
